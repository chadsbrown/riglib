//! Elecraft AI (Auto Information) mode transceive listener.
//!
//! When AI mode is enabled on an Elecraft rig (`AI2;`), the rig pushes unsolicited
...
/// Known AI (Auto Information) prefixes that Elecraft pushes unsolicited.
        ...
        tokio::select! {
            ...
            cmd = cmd_rx.recv() => {
                match cmd {
                    ...
                    None => {
                        // All senders dropped -- ElecraftRig was dropped.
...
/// AI responses.
///
/// This is the transceive-aware equivalent of `ElecraftRig::execute_command`.
...
async fn execute_command_on_transport(
    ...
    for attempt in 0..=retries {
        if attempt > 0 {
            debug!(attempt, "Elecraft CAT command retry (AI mode)");
